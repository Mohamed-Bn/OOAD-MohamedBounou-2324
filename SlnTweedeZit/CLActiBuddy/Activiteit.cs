...
#pragma warning restore SA1011 // Closing square brackets should be spaced correctly
        ...
        public static List<Activiteit> GetActiviteitenByPersoonId(int personId)
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM Activiteit WHERE organisator_id = @id", conn))
                {
        ...
        public void DeleteFromDb()
        {
            ...
            {
                conn.Open();
                SqlCommand comm = new SqlCommand("DELETE FROM Deelname WHERE activiteit_id = @parID ", conn);
            ...
            {
                conn.Open();
                SqlCommand comm = new SqlCommand("DELETE FROM Activiteit WHERE id = @parID", conn);
                comm.Parameters.AddWithValue("@parID", Id);
                comm.ExecuteNonQuery();
            }
        }
